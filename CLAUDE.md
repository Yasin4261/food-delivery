...
- **Schema vs. code:** `migrations/` defines `users`, `chefs`, `menus`, `menu_items`, `orders`, `order_items` (000001–000006), `favorites` (000007) and `reviews` (000008) — all wired into Go.
...
- **Not built:** forgot-password (needs a `password_reset_tokens` table + email), chat, earnings, online/offline toggle, search. Use these as the next features, each following the §2 recipe.
