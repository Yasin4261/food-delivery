...
- **Implemented:**
  ...
  - **Saved cards (#67)** — customers can store cards for one-tap reuse. `payment_methods` (migration 000025) holds **only iyzico references** (`card_user_key`/`card_token`) + display metadata (masked digits, scheme, bank) — **never a PAN/CVC**. `SavedCard` entity + `PaymentMethodRepository` port (idempotent `Add` via `ON CONFLICT (user_id,card_token)`, owner-scoped list/find/delete, `CardUserKey` wallet lookup). The `PaymentGateway` port gains `CheckoutOptions{CardUserKey,RegisterCard}` on `InitiateCheckout`, a `RegisteredCard` on `PaymentResult`, and `DeleteStoredCard`; the mock simulates storage (synthetic card on a save-opted checkout), iyzico passes `cardUserKey` to the hosted form and reads the saved-card fields back on retrieve. `PaymentService` (wired via `SetPaymentMethods`, nil-safe) passes the buyer's wallet key into checkout, **persists a saved card best-effort on payment completion** (never blocking a captured payment), and lists/deletes cards (delete revokes at the gateway first). `POST /api/v2/orders/{id}/pay` takes `{save_card}`; `GET /api/v2/payment-methods` + `DELETE /api/v2/payment-methods/{token}` (auth, owner-scoped → 404 on a foreign token). SPA: saved-cards card on `/profile` (masked list + remove) and a "save this card" checkbox at pay time. TR + EN. **Real-sandbox verification is #51** (needs merchant credentials).
  ...
  - **Currency (#125)** — one ISO-4217 code, `CURRENCY` (default `TRY`, fail-fast on a non-3-letter code), is the **single source of truth**: the iyzico adapter takes it as a constructor arg (previously hardcoded `"TRY"`) and `OrderNotifier` quotes it in emails (`"45.00 TRY"` — deliberately code-not-symbol, since Go has no locale formatter). The SPA renders **every** amount through `web/src/lib/money.js` (`formatMoney`, `Intl.NumberFormat` over the active i18n locale → `₺45,00` in TR, `TRY 45.00` in EN), selected by `VITE_CURRENCY` which must match the API's `CURRENCY`. **Never hardcode a currency symbol** — the bug this fixed was 22 SPA sites + 3 email lines showing `$` while the gateway charged lira.
  ...
  - **Support messaging (#120)** — the chat stack now carries a second thread kind so support conversations inherit the whole live infrastructure (Hub, tagged frames, read receipts, unread badges) instead of duplicating it. `chat_conversations.kind` (`'chef' | 'support'`, migration 000026) + nullable `chef_id`; the old blanket `unique(user_id,chef_id)` becomes two **partial** unique indexes (chef threads one-per-(user,chef); support threads one-per-user) plus a CHECK that a chef thread names a kitchen and a support thread does not. A **support thread is admin↔user** (`user_id` = the customer/chef being helped, `chef_id` NULL, ChefID 0). `Conversation.IsParticipant(userID, chefID, isAdmin)` gained the admin flag: on a support thread the target user **or any admin** is a participant; on a chef thread admins are **deliberately excluded** (support must never read a customer↔chef thread — asserted in tests). `isAdmin` is threaded from the JWT role through `SendMessage`/`Messages`/`MarkRead`/`Authorize` (not a bypass — admin is a genuine participant of *this kind* of thread, encoded in `IsParticipant`). The admin-side unread count reuses `unreadForChef` (messages from the target); the target's uses `unreadForCustomer` (messages from admin) — the existing SQL is expressed purely over `c.user_id`, so both fall out for free. Endpoints: user `POST /api/v2/support/conversations` ("Contact support", idempotent, opens their own thread); admin `GET /api/v2/admin/support/conversations` (inbox) + `POST` (open a thread with `{user_id}`); replies/history/live delivery reuse the shared `…/chat/conversations/{id}/…` endpoints. SPA: a "🛟 Contact support" button for users, and admins get the support inbox merged into their thread list — same messaging UI. TR + EN. **The existing chef↔customer chat, read receipts and unread badges are unchanged (regression-tested).**
  ...
  - **Admin search / filters / pagination (#118)** — the first step of turning the panel from *moderation* into a *support console*. `domain.AdminUserFilters` (`q` over email/username, `role`, tri-state `Active *bool`) / `AdminChefFilters` (`q` over business name, `Active`) / `AdminOrderFilters` (`status`, `payment_status`, `user_id` — "this customer's orders"); the zero value matches everything, preserving prior behaviour. The adapter uses **fixed SQL with bound placeholders only** (`($1 = '' OR col ILIKE '%'||$1||'%')`, `($n::boolean IS NULL OR is_active = $n)`) — never interpolation (§10 policy 2) — and the **same WHERE feeds the `count(*)`**, so the envelope's `total` reflects the filter, not the table. The service **rejects unknown `role`/`status`/`payment_status` with 400** (`ValidationError` → `respondAdminError`) rather than silently returning everything; `domain.ValidOrderStatus`/`ValidPaymentStatus` added alongside the existing `ValidPaymentMethod`. SPA: per-tab filter bar + prev/next pager driven by `total` (filters reset to page 0 on change, so a narrower filter can't strand you past the end). TR + EN.
  - **Admin audit log + write actions (#121/#122)** — the first admin *mutations*, each recorded atomically. `admin_audit_log` (migration 000027; actor, action, target type/id, reason, before/after JSONB) is written **in the same transaction as the change it describes** — the `AdminRepository` owns the tx (`auditBoolToggle`, per-entity promo methods), inserts the row via `insertAudit(ctx, tx, e)`, and commits both together, so a rollback loses both and no unattributed admin change can exist (proven by an integration test that toggles a missing row and asserts **no** audit row is written). All admin writes now flow through the one `AdminRepository` (user/chef active, chef `is_online`/`is_accepting_orders`, promo create/update/delete/active) so mutations, audit and the promo reads share one adapter — `AdminService` dropped its separate `PromoRepository` dep. Chef status is drivable **on the chef's behalf** by id (`PATCH /api/v2/admin/chefs/{id}/status` + `/availability`), distinct from the caller-scoped `/chefs/me/*`. Promo management gains **edit + delete** (`PUT`/`DELETE /api/v2/admin/promos/{id}`) alongside create/active; delete captures a before-snapshot. **Deactivating a user or chef requires a reason** (`ValidationError` → 400) — the destructive actions must be justified in the trail. Read-only audit surface: `GET /api/v2/admin/audit` (filter by `action`/`target_type`/`target_id`/`actor_id`) — **no update/delete endpoint**. Before/after never carry secrets (no hashes/tokens/PAN — §10 A09). SPA: audit tab, chef online/pause controls + badges, promo delete, a reason prompt on deactivation. TR + EN.
  - **Admin order operations (#124)** — support can intervene on a stuck or disputed order, via **named, audited actions** (never a generic field editor — a free-form edit would corrupt the §4 state machines and the snapshotted per-slice money). `OrderService` gains three: **cancel + refund** (`AdminCancelOrder` — reuses the customer-cancel guarantee: a paid card order is refunded through the gateway first and, if the refund fails, the order stays uncancelled so money and state never diverge), **force sub-order status** (`AdminForceSubOrder` — mirrors `AdvanceForChef` incl. the partial-refund-on-decline, but targets any chef's slice by id; illegal transitions still return `ErrInvalidStatusTransition` → 422 for admin too), and **edit delivery snapshot** (`AdminEditDelivery` — address/city/notes, **pre-delivery only** via `Order.IsEditableDelivery`, so history is never rewritten once the food is on its way). Each is atomic with its audit row: `OrderRepository.UpdateStatusAudited`/`UpdateSubOrderAudited` add an optional `*AuditEntry` inserted (via the shared `insertAudit`) inside the existing update transaction — proven by an integration test that counts `admin_audit_log` rows growing in step. `POST /api/v2/admin/orders/{id}/cancel`, `POST /…/status` (`{chef_id,action,reason}`), `PUT /…/delivery`, all `RequireRole(admin)`. **Goodwill credit is deliberately out of scope** — it needs its own credit-ledger feature, not an order-total rewrite. SPA: cancel/edit-delivery buttons + a per-slice force-status control in the order detail card. TR + EN.
  - **Admin profile editing (#123)** — an admin edits a customer's contact/location or a chef's kitchen **on their behalf** (support: "fix my address typo"), each atomic with an audit entry. `AdminRepository.UpdateUserProfile`/`UpdateChefProfile` do a `FOR UPDATE` before-read + an UPDATE that touches **only editable columns** — email/username/role/`password_hash` are never in the statement, so they are immutable by construction (an integration test re-reads the row and asserts identity + hash are byte-identical after the edit). The handler additionally **rejects** a request that even *carries* `email`/`username`/`role`/`password` (400), so the immutability is explicit not just incidental (a test per field). `PUT /api/v2/admin/users/{id}` (contact/location + email-notif pref) + `PUT /api/v2/admin/chefs/{id}` (business name, kitchen address/city/coords, delivery radius); the updated entity is returned with the hash cleared. SPA: an "Edit" form inside the detail card (#119). TR + EN.
  - **Admin detail views (#119)** — read-only support drill-in, because support is mostly *reading context*: `GET /api/v2/admin/{users,orders,chefs}/{id}` return `domain.AdminUserDetail` (account + kitchen if any + recent orders + reviews written), `AdminOrderDetail` (order incl. items/sub-orders + customer + **every payment attempt**, newest first — a failed-then-retried payment is what tickets are about) and `AdminChefDetail` (kitchen + owner + dishes + recent orders). Nested lists capped at `domain.AdminDetailLimit` (20) so one lookup can't pull thousands of rows. The adapter composes by reusing the sibling repositories (`&OrderRepository{db: r.db}` etc.) but has its **own unfiltered chef lookups** — `ChefRepository.FindByID` filters `is_active = true`, so support could otherwise never open a **deactivated** kitchen, which is exactly who they need to inspect. The service clears `PasswordHash` on every embedded user; `PaymentSession.Token`/`PaymentID` stay `json:"-"` so gateway secrets never reach the browser (asserted in tests). SPA: clicking any row opens a detail card (row-action buttons use `@click.stop`). TR + EN.
...
- **Schema vs. code:** `migrations/` 000001–000027 define `users`, `chefs` (+`is_online`), `menus`, `menu_items`, `orders` (+`tip`), `order_items`, `favorites`, `reviews`, `password_reset_tokens`, `chat_conversations` (+`kind` for support threads), `chat_messages`, `payment_sessions`, `sub_orders` (+`tip`), `promo_codes`, `email_verification_tokens`, `payment_methods`, plus pg_trgm search indexes — all wired into Go.
...
| `v4.21.1` | config fix: `CURRENCY` is now settable in Docker — one var drives both the API and the SPA build (`VITE_CURRENCY`) so they can't diverge; default stays `TRY` (#137, PR #138) |
...
|---|---|
| **A01 Broken Access Control** | Role guards at the router (`handleAuth` / `handleRole(chef)`); **per-resource ownership enforced in services** (`domain.ErrForbidden` → 403): chefs own their menus/dishes, customers their orders/reviews, chat is participant-only — and **admins are participants of support threads only, never of customer↔chef threads** (#120). `admin` cannot be self-assigned at registration. |
...
| **A09 Logging & Monitoring Failures** | Structured `slog` request logs (request id, status, latency); unexpected 500s logged server-side. **Admin audit trail** (#121, `admin_audit_log`): every admin mutation writes an immutable actor/action/target/before/after row in the same transaction as the change — no update/delete endpoint, before/after never carry secrets. Prometheus `/metrics` (#73, `internal/metrics`) — HTTP rate/latency/status, Go+process+DB-pool, order/payment counters — served on the internal port only (Caddy 404s the public path); `monitoring` compose profile adds Prometheus + Grafana + alert rules (5xx, p95, DB, API-down). **Never log credentials, JWTs or reset tokens** — the dev logging mailer prints reset links *in development only* by design. |
