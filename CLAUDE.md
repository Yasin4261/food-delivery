...
- **Implemented:**
  ...
  - **Email notifications (#58)** — `service.OrderNotifier` (reuses the `domain.Mailer` port): "new order" to each participating chef on placement (their slice only) and "status changed" to the customer on meaningful sub-order transitions (confirmed/delivering/delivered/declined — not preparing/ready). **Fire-and-forget**: goroutines with `context.WithoutCancel`, failures logged (`order email failed`), never surfaced to the buyer; nil notifier disables. No new config — SMTP in prod, dev logging mailer otherwise.
...
- **Schema vs. code:** `migrations/` 000001–000015 define `users`, `chefs` (+`is_online`), `menus`, `menu_items`, `orders`, `order_items`, `favorites`, `reviews`, `password_reset_tokens`, `chat_conversations`, `chat_messages`, `payment_sessions`, `sub_orders`, plus pg_trgm search indexes — all wired into Go.
