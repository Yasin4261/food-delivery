...
- **Schema vs. code:** `migrations/` 000001–000021 define `users`, `chefs` (+`is_online`), `menus`, `menu_items`, `orders`, `order_items`, `favorites`, `reviews`, `password_reset_tokens`, `chat_conversations`, `chat_messages`, `payment_sessions`, `sub_orders`, plus pg_trgm search indexes — all wired into Go.
