...
- **Product features: complete.** All feature issues (#1–#9) are implemented. Remaining open work is tech-debt/security (#10–#12, #14–#18): JWT secret handling, token revocation, rate limiting, HTTP timeouts/graceful shutdown, CORS, structured logging, pagination metadata, stale-docs cleanup — and real email delivery (#email-infra) to replace the password-reset stub.
