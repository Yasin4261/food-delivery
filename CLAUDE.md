...
- **Schema vs. code:** `migrations/` defines `users`, `chefs`, `menus`, `menu_items`, `orders`, `order_items` — all six are now wired into Go.
...
- **Not built:** forgot-password (needs a `password_reset_tokens` table + email), favorites, reviews, chat, earnings, online/offline toggle. Use these as the next features, each following the §2 recipe.
