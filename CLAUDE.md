...
- **Implemented:**
  ...
  - **Chef earnings** — derived (no ledger): `EarningsRepository` sums the chef's **delivered slices on paid orders** (sub-order gate), optional `since` window. `GET /api/v2/chefs/me/earnings?days=N` (chef) returns gross food subtotal, delivery fees (kept in full), commission and **net**.
...
- **Schema vs. code:** `migrations/` 000001–000023 define `users`, `chefs` (+`is_online`), `menus`, `menu_items`, `orders`, `order_items`, `favorites`, `reviews`, `password_reset_tokens`, `chat_conversations`, `chat_messages`, `payment_sessions`, `sub_orders`, `promo_codes`, `email_verification_tokens`, plus pg_trgm search indexes — all wired into Go.
