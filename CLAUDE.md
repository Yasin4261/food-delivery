...
- **Implemented:**
  ...
  - **Forgot/reset password** — `password_reset_tokens` (migration 000011, stores only sha256 of the token); `PasswordResetRepository`; `AuthService.RequestPasswordReset` (single-use, expiring, silent for unknown emails) + `ResetPassword`. `POST /api/v2/auth/forgot-password` (always 202) + `/reset-password`. Email delivery is stubbed (token logged; echoed only when `Env != production`) pending #email-infra.
...
- **Features + hardening: complete.** All feature issues (#1–#9) and the tech-debt/security issues (#10–#12, #14–#18) are implemented. The only known follow-up is real email delivery (#email-infra) to replace the password-reset stub (and, for multi-instance deploys, moving the token denylist + rate limiter to a shared store like Redis).
